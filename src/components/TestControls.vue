...
async function send(delay: number) {
  ...
  try {
    ...
    lastResult.value =
      delay > 0
        ? `Scheduled, pending_id=${(r as { pending_id?: number }).pending_id}. Lock your phone now.`
...
async function copyUrl() {
  ...
  lastResult.value = "Trigger URL copied. Open it from any other device to push to this browser.";
}
</script>
